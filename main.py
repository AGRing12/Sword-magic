...
BOSS_1_5_EXP = (50, 50)
BOSS_6_10_EXP = (100, 100)
BOSS_1_5_GOLD = (50, 50)
BOSS_6_10_GOLD = (100, 100)
SECRET_BOSS_EXP = 200
SECRET_BOSS_GOLD = 200
...
WEAPONS = {
    ...
    "wood": {"price": 100, "range": 5, "dmg": 0, "mult": 1.0, "label": "Wood Sword"},
    "twin_daggers": {"price": 200, "range": -2, "dmg": 0, "mult": 2.0, "label": "Twin Daggers"},
    "long_sword": {"price": 500, "range": 10, "dmg": 3, "mult": 1.5, "label": "Long Sword"},
    "war_axe": {"price": 700, "range": 5, "dmg": 9.5, "mult": 1.3, "label": "War Axe"},
    "war_hammer": {"price": 800, "range": 5, "dmg": 7.3, "mult": 1.5, "label": "War Hammer"},
    "god_sword": {"price": 2500, "range": 30, "dmg": 50, "mult": 5.0, "label": "God Sword"},
}
ARMORS = {
    "none": {"price": 0, "hits": 0, "label": "None"},
    "light": {"price": 100, "hits": 2, "label": "Light"},
    "medium": {"price": 200, "hits": 4, "label": "Medium"},
    "heavy": {"price": 400, "hits": 6, "label": "Heavy"},
    "super": {"price": 800, "hits": 10, "label": "Super"},
    "god": {"price": 3000, "hits": 20, "label": "God"},
}
...
def drop_rewards(state: GameState, is_boss: bool, stage: int):
    ...
    if is_boss:
        if stage <= 5:
            gold = random.randint(*BOSS_1_5_GOLD)
            exp = random.randint(*BOSS_1_5_EXP)
        elif stage <= 10:
            gold = random.randint(*BOSS_6_10_GOLD)
...
def draw_enemy(surface, e: Enemy, state=None):
    # Use sprite for Imp if available
...
def draw_stats_page(surface, font_small, player: Player, state: GameState):
    ...
    surface.blit(basic_header, (100, y_offset))
    y_offset += 40
    
    # Player stats
    stats_text = [
        f"Health: {player.hp}/{player.max_hp}",
        f"Attack: {player.damage}",
    ...
    for stat in stats_text:
        ...
        surface.blit(stat_surface, (120, y_offset))
        y_offset += 25
    
    # Equipment Section
    y_offset += 20
    equipment_header = font_header.render("Equipment", True, YELLOW)
    surface.blit(equipment_header, (100, y_offset))
    y_offset += 40
    
    # Weapon info
    weapon_text = font_stat.render(f"Weapon: {weapon['label']}", True, WHITE)
    surface.blit(weapon_text, (120, y_offset))
    y_offset += 25
    
    weapon_stats = [
        f"  Damage: {weapon['damage']}",
        f"  Range: {weapon['range']}",
        f"  Speed: {weapon['speed']:.2f}",
        f"  Cost: {weapon['cost']} gold"
    ]
    
    for stat in weapon_stats:
        stat_surface = font_stat.render(stat, True, (200, 200, 200))
        surface.blit(stat_surface, (140, y_offset))
        y_offset += 20
    ...
    armor_text = font_stat.render(f"Armor: {armor_display}", True, WHITE)
    surface.blit(armor_text, (120, y_offset))
    y_offset += 25
...
def main():
    ...
    while True:
        ...
        # Draw stats page if enabled
        if state.show_stats:
            draw_stats_page(screen, font_small, state.player, state)
